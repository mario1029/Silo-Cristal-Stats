const queries = {
  ...
  UPDATE_ACTIVITY=`UPDATE actividad SET codigo_instrument=$2, hora_inicio= CAST($3 AS TIME), hora_fin=CAST($4 AS TIME), fecha= CAST($5 AS DATE) WHERE codigo_actividad= $1 RETURNING *`,
  ...
  UPDATE_MAINTENANCE=`UPDATE mantenimiento SET codigo_instrument=$2, hora_inicio= CAST($3 AS TIME), hora_fin=CAST($4 AS TIME), fecha= CAST($5 AS DATE), motivo=$6 WHERE codigo_actividad= $1 RETURNING *`,
  DELETE_MAINTENANCE=`DELETE * FROM mantenimiento WHERE codigo_actividad=$1`,
  ...
  UPDATE_LOSS=`UPDATE falla SET codigo_instrument=$2, hora_inicio= CAST($3 AS TIME), hora_fin=CAST($4 AS TIME), fecha= CAST($5 AS DATE), motivo=$6, perdida=$7 WHERE codigo_actividad= $1 RETURNING *`,
  DELETE_LOSS=`DELETE * FROM falla WHERE codigo_actividad=$1`,
